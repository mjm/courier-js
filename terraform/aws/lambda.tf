...
resource "aws_iam_role_policy_attachment" "ping_basic" {
  role       = aws_iam_role.ping.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_role_policy_attachment" "ping_table" {
  role       = aws_iam_role.ping.name
  policy_arn = aws_iam_policy.courier_table.arn
}

resource "aws_iam_role_policy_attachment" "ping_config" {
  role       = aws_iam_role.ping.name
  policy_arn = aws_iam_policy.config_params.arn
}

resource "aws_lambda_function" "ping" {
  function_name = "ping-${var.env}"
  handler       = "ping"
  runtime       = "go1.x"
  role          = aws_iam_role.ping.arn

  s3_bucket = aws_s3_bucket.lambda_handlers.bucket
  s3_key    = "${var.function_revision}/ping.zip"
