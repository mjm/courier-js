...
const (
	...
	PostsRecent = "SELECT id, item_id, text_content, html_content, title, url, published_at, modified_at FROM posts WHERE feed_guid = $1 ORDER BY published_at DESC LIMIT 10;"

	// tweets.sql
	TweetsByPostIDs = "SELECT * FROM tweets WHERE feed_subscription_guid = $1 AND post_id = ANY ($2) ORDER BY post_id, position;"
